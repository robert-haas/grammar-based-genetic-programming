...
public class ContextualDerivationTree extends DerivationTree{
    ...
    {
        super(gram, gen);
        logger.log(Level.INFO, ">> ContextualDerivationTree: constructor called");
    ...
    {
        logger.log(Level.INFO, ">> ContextualDerivationTree: growNode()");
        Symbol s = t.getData();
        logger.log(Level.INFO, "  Symbol: {0}", s);
        boolean found = false;
        ...
        for(Symbol NTSymbol : nonTerminalRules)
        {
            if(s.equals(NTSymbol))
            {
                ...
                logger.log(Level.INFO, "  is NT => into structCodonList");
                found = true;
            }
        }
        ...
        if(usedCodon)
        {
            for(Symbol TSymbol : terminalRules)
            {
                ...
                {        
                    ...
                    logger.log(Level.INFO, "  is T => into nodeCodonList");
                }
            }
        }
        logger.log(Level.INFO, "  structCodonList: {0}", this.structCodonList);
        logger.log(Level.INFO, "  nodeCodonList: {0}", this.nodeCodonList);
