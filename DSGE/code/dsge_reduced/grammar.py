...
class Grammar:
    ...
    def recursive_individual_creation(self, genome, symbol, size_of_gene, current_depth):
        if current_depth > self.max_init_depth:
            ...
            print(symbol, expansion_possibility)
    ...
    def recursive_mapping(self, mapping_rules, positions_to_map, current_sym, current_depth, output):
        LOGGER.info('  Called: recursive_mapping(current_depth=%s)', current_depth)
        depths = [current_depth]
        if current_sym[1] == self.T:
            LOGGER.info('    Terminal symbol: %s', current_sym[0])
            output.append(current_sym[0])
        else:
            LOGGER.info('    Nonterminal symbol: %s', current_sym[0])

            current_sym_pos = self.ordered_non_terminals.index(current_sym[0])
            choices = self.grammar[current_sym[0]]
            size_of_gene = self.count_number_of_options_in_production()

            LOGGER.info('    Rules: %s ::= %s', current_sym[0], _productions_to_str(choices))
            LOGGER.info('      Number of possible productions: %s', len(choices))

            if positions_to_map[current_sym_pos] >= len(mapping_rules[current_sym_pos]):
                LOGGER.info(
                    '          Reached the limit of choices that can be decided by '
                    'the integers in the gene: %s >= %s',
                    positions_to_map[current_sym_pos],
                    len(mapping_rules[current_sym_pos]))
                if current_depth > self.max_depth:
                    ...
                    for index, option in enumerate(self.grammar[current_sym[0]]):
                        for s in option:
                            ...
                            possibilities.append(index)
                    expansion_possibility = random.choice(possibilities)
                    LOGGER.info('          Genotype repair: Randomly created integer '
                                '(from limited possibilities due to maximum depth '
                                'being reached): %s',
                                expansion_possibility)
                else:
                    expansion_possibility = random.randint(0, size_of_gene[current_sym[0]] - 1)
                    LOGGER.info('          Genotype repair: Randomly created integer: %s',
                                expansion_possibility)
            ...
            LOGGER.info('        Used gene (corresponds to current nonterminal): %s',
                        current_sym_pos)
            LOGGER.info('        Used integer position within this gene: %s',
                        positions_to_map[current_sym_pos])
            LOGGER.info('        Used integer: %s', current_production)
            LOGGER.info('        Chosen rule index: %s', current_production)
            LOGGER.info('        Production: %s',
                        _production_to_str(choices[current_production]))

            positions_to_map[current_sym_pos] += 1

            LOGGER.info('      Number of used integer positions in this gene: %s',
                        positions_to_map[current_sym_pos])
...
def _production_to_str(production):
    """Helper function to convert a single production to a string for logging."""
    return ''.join(sym[0] for sym in production)


def _productions_to_str(productions):
    """Helper function to convert multiple productions to a string for logging."""
    return ' | '.join(_production_to_str(pr) for pr in productions)
